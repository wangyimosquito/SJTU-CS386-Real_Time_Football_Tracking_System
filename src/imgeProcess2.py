...
def initialPlayers(players_pos ,playerKalman, playerrec, contours, hueColor, HuePrediction):
	for i in range (20):
		...
		for j in range(2): #卡尔曼滤波初始化循环2次初始值坐标
			playerKalman[i].update(playerrec[i][0] + playerrec[i][2]//2,playerrec[i][1] + playerrec[i][3]//2)
...
def PlayertoContour(players_pos, playerKalman, HuePrediction, Count):
	for i in range(len(players_pos)):
		...
		if(w<5 and h<5):#球或其他过小目标忽略
			continue
		if(y+h> oripoints[0][1]-10 + h or y<oripoints[1][1]+10):#忽略边界外的裁判和巡线员
			continue
		singleFlag = False
		if(w<=30 and h <=30):
			singleFlag = True
		if(singleFlag == True):#单人线框选择
			...
			for j in range(20):
				if(playerKalman[j].oocIndex == -1):
					...
					currSpeed = [x-px,y-py]
					penal = 300
					if(px>x-5 and px< x+w+10 and py>y-5 and py<y+h+10):
						penal = np.sqrt(np.square(px-(x+w//2))+np.square(py-(y+h//2)))
					#huepenal = 0.3*abs(HuePrediction[i]-playerKalman[j].hue)
					huepenal = 0
					KalmanEst = playerKalman[j].get_estimate()
					KalmanPenal = np.sqrt(np.square(KalmanEst[0]-(x+w//2))+np.square(KalmanEst[1]-(y+h//2)))
					...
					if(score[j] < bestScore):
						...
						bestScore = score[j]
			if(bestScore < 300):
				playerKalman[bestMatch].update(x+w//2,y+h//2)
			...
			for j in range(20):
				if(playerKalman[j].oocIndex == -1):
					...
					currSpeed = [x-px,y-py]
					penal = 1000
					if(px>x-3 and px< x+w+6 and py>y-3 and py<y+h+6):
						penal = np.sqrt(np.square(px-(x+w//2))+np.square(py-(y+h//2)))
					...
					if(Count<=50 and px>x-10 and px < x+w+10 and py>y-10 and py<y+h+10):
						playerKalman[j].update(x+w//2,y+h//2)
						playerKalman[j].oocIndex = i
						# playerKalman[j].w = w
						# playerKalman[j].h = h
						playerKalman[j].w = 10
	...
	if(Count < 20):#小于20帧，由于卡尔曼滤波预测还不稳定，因此用上一帧轮廓代替当前帧轮廓
		for i in range(20):
			if(playerKalman[i].oocIndex == -1):
				...
				players_pos.append(ShowContours(text = str(i),contours = [int(playerKalman[i].mp[0]-playerKalman[i].w//2),int(playerKalman[i].mp[1]-playerKalman[i].h//2),int(playerKalman[i].w),int(playerKalman[i].h)]))
		...
		for i in range(20):
			if(playerKalman[i].oocIndex == -1):
				...
				for j in range(originallen):
					...
					if(w<5 and h<5):#球或过小目标忽略
						continue
					if(y+h>oripoints[0][1]-10 + h or y<oripoints[1][1]+10):#忽略边界巡线员
						continue
					KalmanPenal = np.sqrt(np.square(KalmanEst[0]-(x+w//2))+np.square(KalmanEst[1]-(y+h//2)))
					penal = 300
					if(px>x-3 and px < x+w+6 and py>y-3 and py<y+h+6):
						penal = np.square(px-(x+w//2))+np.square(py-(y+h//2))
					bonus = 0#对于暂时还未分配识别目标给予正向奖励
					if(players_pos[j].text == "" and playerKalman[i].lastUnMove == 1):
						bonus -= 50
				...
				if(bestScore < 300):#存在合适的轮廓加入
					if(players_pos[bestMatch].text == ""):#单人轮廓框，直接用该轮廓框进行卡尔曼滤波更新
						playerKalman[i].update(players_pos[bestMatch].contours[0] + players_pos[bestMatch].contours[2]//2,players_pos[bestMatch].contours[1] + players_pos[bestMatch].contours[3]//2)
				...
				else:#不存在一个比较合适的轮廓加入,以卡尔曼滤波预测的选框加入，但不update
					est = playerKalman[i].get_estimate()
					# players_pos.append(ShowContours(text = str(i),contours = [int(playerKalman[i].mp[0]-playerKalman[i].w//2),int(playerKalman[i].mp[1]-playerKalman[i].h//2),int(playerKalman[i].w),int(playerKalman[i].h)]))
					players_pos.append(ShowContours(text = str(i),contours = [int(est[0]-playerKalman[i].w//2),int(est[1]-playerKalman[i].h//2),int(playerKalman[i].w),int(playerKalman[i].h)]))
					playerKalman[i].oocIndex = len(players_pos)-1
					playerKalman[i].lastUnMove = 1
	return players_pos, playerKalman
...
def drawContour(players_pos, playerKalman, selectedPlayer, img, playerpos):
	for i in range(len(players_pos)):
		...
		if(players_pos[i].text != ""):
			if(playerKalman[selectedPlayer].oocIndex == i):
				...
				cv2.circle(img, (x+w//2,y+h//2),2,(0,0,255),-1)#对于当前选择的球员，其质心为红色
	...
	#显示20名球员选框
	# for i in range(20):
	# 	x= int(playerKalman[i].mp[0])
	# 	y = int(playerKalman[i].mp[1])
	# 	w = int(playerKalman[i].w)
	# 	h = int(playerKalman[i].h)
	# 	if(selectedPlayer == i):
	# 		cv2.rectangle(img, ((x-w//2),(y-h//2)), ((x+w//2), (y+h//2)), (0, 0, 255), 1)#对于当前选择的球员，其轮廓为红色
	# 		#cv2.circle(img, (x,y),2,(0,0,255),-1)#对于当前选择的球员，其质心为红色
	# 		cv2.putText(img, str(i), (x,y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, (255,255,255))#显示球员字号
		# else:
		# 	#cv2.rectangle(img, ((x-w//2),(y-h//2)), ((x+w//2), (y+h//2)), (255, 255, 255), 1)#对于当前选择的球员，其轮廓为红色
		# 	cv2.circle(img, (x,y),2,(255,255,255),-1)#对于当前选择的球员，其质心为红色
		# 	cv2.putText(img, str(i), (x,y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, (255,255,255))#显示球员字号
		#playerpos.append([(x,y),str(i)])
...
#在俯视图中按照20player画出轮廓
def drawtopView2(players_pos, playerKalman ,playerpos, hg_matrix, Count, originalFps, frame_count, selectedPlayer):
	if(Count == 0):
		for i in range(len(players_pos)):
			if(players_pos[i].text != ""):
				center = [players_pos[i].contours[0]+players_pos[i].contours[2]//2,players_pos[i].contours[1]+players_pos[i].contours[2]//2]
	...
	top_img = topview.create_topview(hg_matrix, playerpos, selectedPlayer)
	#获取当前时间
	now_seconds=int(Count /originalFps%60)
	now_minutes=int(Count/originalFps/60)
	total_second=int(frame_count /originalFps%60)
...
def PlayertoContour2(players_pos, playerKalman, HuePrediction, Count):
	for i in range(len(players_pos)):
		(x,y,w,h) = players_pos[i].contours
		if(w<5 and h<5):#球或其他过小目标忽略
			continue
		...
		if(singleFlag == True):#单人线框选择
			...
			for j in range(20):
				if(playerKalman[j].oocIndex == -1):
					...
					#huepenal = 0.3*abs(HuePrediction[i]-playerKalman[j].hue)
					#矩形重叠面积计算Penal值
					maxA, overlapArea = overlapRec(px-5, px+5,py-7.5,py+7.5,x,x+w,y,y+h)
					
					penal = (1-overlapArea/maxA)*300
					huepenal = 0
					KalmanEst = playerKalman[j].get_estimate()
					KalmanPenal = np.sqrt(np.square(KalmanEst[0]-(x+w//2))+np.square(KalmanEst[1]-(y+h//2)))
					score[j] = penal + np.sqrt(np.square(currSpeed[0]-playerKalman[j].speed[0])+np.square(currSpeed[1]-playerKalman[j].speed[1])) + huepenal + KalmanPenal
			...
			for j in range(20):
				if(playerKalman[j].oocIndex == -1):
					...
					penal = (1-overlapArea/maxA)*300
					if(overlapArea > 0):
						print("overlap: ",overlapArea, "penal: ",penal, "minArea",maxA)
					...
					if(Count<=50 and currscore<400):
						...
						if (players_pos[i].text != ""):
							players_pos[i].text += ","
						players_pos[i].text += str(j)
					elif(Count>50 and currscore < 300):#50帧之后卡尔曼滤波打分存在参考价值
						playerKalman[j].update(KalmanEst[0],KalmanEst[1])
						playerKalman[j].oocIndex = i
						# playerKalman[j].w = w
						# playerKalman[j].h = h
...
#计算两矩形重合面积
def overlapRec(ax1,ax2,ay1,ay2,bx1,bx2,by1,by2):
	area1 = (ax2 - ax1) * (ay2 - ay1)
	area2 = (bx2 - bx1) * (by2 - by1)
	overlapWidth = min(ax2, bx2) - max(ax1, bx1)
	overlapHeight = min(ay2, by2) - max(ay1, by1)
	overlapArea = max(overlapWidth, 0) * max(overlapHeight, 0)
	return min(area1,area2),overlapArea




def ContourtoPlayer2(players_pos, playerKalman, HuePrediction,Count):
	if(Count < 20):#小于20帧，由于卡尔曼滤波预测还不稳定，因此用上一帧轮廓代替当前帧轮廓
		for i in range(20):
			if(playerKalman[i].oocIndex == -1):
				playerKalman[i].update(int(playerKalman[i].mp[0]),int(playerKalman[i].mp[1]))
